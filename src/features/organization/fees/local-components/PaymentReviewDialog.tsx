...
export function PaymentReviewDialog({
  ...
  function renderLineItems() {
    ...
    const renderRow = (item: PaymentReviewLineItem, index: number) => (
      <div key={item.label + "-" + index} className="flex items-center justify-between px-3 py-2">
             ...
             {data.paymentMethod !== "cash" && data.receiptContent && (
              ...
              <DialogFooter className="flex-col gap-2 sm:flex-row">
                {isPending ? (
                  ...
                  <Button variant="outline" className="text-black gap-1.5 bg-linear-to-r from-[#8BC34A] via-[#6ac947] to-[#55c72c] " onClick={() => onOpenChange(false)}>
                    Close
                  </Button>
                    </>
                )}
