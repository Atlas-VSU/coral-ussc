...
export function MembersStats({
  ...
  const [eventPresentCounts, setEventPresentCounts] = useState<{ [key: string]: number }>({});
  ...
  }, [eventAttendance, filterType, years, studentStats.totalStudents, eventPresentCounts]);
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Select value={filterType} onValueChange={setFilterType}>
                ...
                <SelectContent>
                  <SelectItem value="recent"><span className="text-xs">Recent Events</span></SelectItem>
                  <SelectItem value="all"><span className="text-xs">All Events</span></SelectItem>
                  {years.map((year) => (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
              {chartData.map((event, index) => {
                ...
                return (
                  ...
                  >
                    <p className="text-sm font-medium text-foreground mb-3">{event.displayName}</p>
                    <div className="relative">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span className="text-base font-bold text-foreground">{event.attendanceRate}%</span>
                      </div>
                    </div>
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <XAxis
                    dataKey="displayName"
                    tick={{ fontSize: 12, fill: chartColors.textColor, fontWeight: 500 }}
                  ...
                  <Legend
                    wrapperStyle={{ paddingTop: "16px", fontSize: "13px", fontWeight: 500, color: chartColors.foregroundColor }}
                    iconSize={10}
                    iconType="circle"
                  />
                  <Bar dataKey="Present" fill={COLORS.present} name="Present" radius={[4, 4, 0, 0]} animationDuration={1200} />
                  <Bar dataKey="Absent" fill={COLORS.absent} name="Absent" radius={[4, 4, 0, 0]} animationDuration={1200} animationBegin={300} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
          ...
          {!isLoading && chartData.length > 1 && (
            <div className="mt-4 pt-4 border-t border-border">
              ...
              <div className="flex flex-wrap gap-2">
                {chartData.map((event, index) => {
                  const colors = [
                    ...
                    "hsl(17 87% 59%)", "hsl(142 69% 58%)", "hsl(199 89% 48%)",
                    "hsl(280 87% 65%)", "hsl(25 95% 53%)",
                  ];
              ...
              <p className="text-xs text-muted-foreground mt-3">
                ...
                {chartData.reduce((sum, e) => sum + (e.Present || 0), 0)} total attendees
              </p>
            </div>
          )}
