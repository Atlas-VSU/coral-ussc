...
interface PaymentsFiltersProps {
  ...
  showStatusFilter?: boolean;                // For unpaid tab (no status filter)
}
...
export function PaymentsFilters({
  ...
  const hasActiveFilters = searchTerm || (showStatusFilter && statusFilter !== "all");
      ...
      <div className="flex flex-col gap-3 lg:hidden">
        <div>
          ...
          <p className="text-xs text-gray-500 hidden sm:block">
            Find payments by name, ID{showStatusFilter ? ", or filter by status" : ""}
          </p>
        </div>
